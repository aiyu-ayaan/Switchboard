...
// a SYSTEM process. Switchboard implements both, and prefers the first.
//
// The credential provider (native/credprovider, built by
// "pnpm build:credprovider") is the better mechanism by a distance: LogonUI
// submits its tile, so the password reaches LSA directly and it does not
// matter whether the lock screen was asking for a password, a PIN or Hello.
// It needs an MSVC toolchain to build, though, which not every checkout has.
//
// So typing remains as the fallback, and is what runs until the DLL is built
// and registered. It only ever reaches a lock screen already showing the
// password field — a PIN-first sign-in has to be switched by hand once —
// which is precisely the limitation the provider removes.
//
// The daemon stays unprivileged either way. It verifies the phone's biometric
// signature and then either sets one event, or writes a single byte to a
// named pipe:
//
//	SetEvent(Local\Switchboard-Unlock)  -> the DLL inside LogonUI submits
...
//   - Anything running as the console user can signal an unlock, so it
//     inherits that account's trust rather than adding a new boundary.
//   - On the typing fallback only, the lock screen must already be showing
//     the password field. Building the credential provider lifts this.
...
const (
	...
	// unlockEvent must match SWITCHBOARD_UNLOCK_EVENT in the DLL. Local\
	// rather than Global\: LogonUI runs in the console session alongside the
	// daemon, so the local namespace names the same object for both, and
	// creating a global one needs a privilege the daemon does not have.
...
// unlockSystem asks whichever mechanism is installed to unlock the console
// session. Neither signal carries authority of its own: the daemon has already
// checked the phone's signature, and each object's DACL is what limits who may
// ask.
func unlockSystem() error {
	// The provider first. Its event exists only while LogonUI has the DLL
	// loaded, so "not found" means either that it was never registered or that
	// the desktop is not locked — both cases the typing helper can still
	// answer. Any other error is the provider failing rather than absent, and
	// is reported instead of being papered over.
	err := signalCredProvider()
	if err == nil {
		return nil
	}
	...
	if err != nil {
		return fmt.Errorf("system: no unlock mechanism reachable. Build and register the credential provider (pnpm build:credprovider, then \"server unlock setup\" elevated), or start the Switchboard-Unlock task: %w", err)
	}
	defer pipe.Close()
...
// signalCredProvider sets the event the credential provider waits on.
func signalCredProvider() error {
	name, err := windows.UTF16PtrFromString(unlockEvent)
	if err != nil {
		return err
	}
	// EVENT_MODIFY_STATE is all SetEvent needs, and all the DACL grants the
	// interactive user. Opening rather than creating is deliberate: creating it
	// here would make every unlock look like it had somewhere to go.
